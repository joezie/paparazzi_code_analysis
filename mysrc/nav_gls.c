...
static inline bool gls_compute_TOD(uint8_t _af, uint8_t _sd, uint8_t _tod, uint8_t _td)
{
  ...
  //Bound(wind_on_final, -MAX_WIND_ON_FINAL, MAX_WIND_ON_FINAL); // joezie 2019/10/29
...
bool gls_run(uint8_t _af, uint8_t _sd, uint8_t _tod, uint8_t _td)
{
  ...
  Bound(nav_final_progress, -1, 1); // joezie 2019/10/29
  ...
  //Bound(nav_intercept_progress, -1, 1); // joezie 2019/10/29
  ...
  //Bound(pre_climb, -5, 0.); // joezie 2019/10/29
